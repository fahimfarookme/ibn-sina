...
module.exports = function(grunt) {
   ...
   // build properties
   var config = (function() {
      var build = grunt.file.readJSON(".ibn-sinarc");
      if (!build) {
         grunt.fail.fatal([".ibn-sinarc doesnt exists"]);
      }
      ...
      if (!build.optimize) {
         grunt.log.error([".ibn-sinarc doesnt define optimize. Commnad line options or defaults will be used."]);
      ...
      if (!(build.src && build.test && build.dist && build.temp)) {
         grunt.fail.fatal([".ibn-sinarc doesnt define mandatory directories: src, test, dist, temp"]);
      }
